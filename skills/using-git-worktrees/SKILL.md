...
确保工作在隔离的工作区中进行。优先使用你平台的 worktree 原生工具。仅在没有原生工具可用时才回退到手动 git worktree。

**核心原则：** 先检测已有的隔离。然后使用原生工具。然后回退到 git。绝不要与 harness 对抗。
...
原生工具自动处理目录放置、分支创建和清理。当你拥有原生工具时使用 `git worktree add` 会创建你的 harness 无法看见或管理的幻影状态。
