...
当你修复了一个由无效数据引起的 bug 时，只在一个地方添加验证感觉就足够了。但那一个检查可以被不同的代码路径、重构或 mock 绕过。

**核心原则：** 在数据经过的每一层都进行验证。让 bug 变得在结构上不可能发生。

## 为什么需要多层

单层验证："我们修复了 bug"
多层验证："我们让 bug 变得不可能发生"
...
async function gitInit(directory: string) {
  ...
  if (process.env.NODE_ENV === 'test') {
    ...
    if (!normalized.startsWith(tmpDir)) {
      throw new Error(
        `在测试期间拒绝在临时目录之外进行 git init: ${directory}`
      );
    }
  }
  // ... 继续
